...
# ── Stripe API retry helper ──────────────────────────────────────────────────
import time

def _stripe_call(fn, *args, **kwargs):
    """Call a Stripe API function with exponential backoff on rate limit (429)."""
    for attempt in range(5):
        try:
            return fn(*args, **kwargs)
        except stripe.error.RateLimitError as e:
            wait = 2 ** attempt
            print(f"  Stripe rate limit hit — retrying in {wait}s (attempt {attempt + 1}/5)")
            time.sleep(wait)
    raise RuntimeError("Stripe API rate limit: all retries exhausted")
...
def stripe_status_to_label(status):
    label = _KNOWN_STRIPE_STATUSES.get(status)
    if label is None:
        print(f"  Warning: unknown Stripe status '{status}' — treating as 'Unknown'")
        return "Unknown"
    return label
...
def fetch_all_subscriptions():
    ...
    while True:
        page = _stripe_call(stripe.Subscription.list, **params)
...
def _compute_projections(sub_dict, amount_usd, interval):
    ...
    if interval == "Annual":
        ...
        if not ts_end:
            return proj
        # Monthly: compute the billing day-of-month from the anchor date,
        # then place charges directly into every month within the window.
        dt_anchor = datetime.fromtimestamp(int(ts_end), tz=timezone.utc)
        billing_day = dt_anchor.day
        for month_idx in range(12):
            year  = 2026
            month = month_idx + 1
            day   = min(billing_day, calendar.monthrange(year, month)[1])
            proj[month_idx] = round(amount_usd, 2)
    ...
    for sub in subs:
        ...
        if isinstance(cust, str):
            ...
            try:
                ...
                country = country or ""
                print(f"  Warning: could not parse customer fields for {cust_id!r}: {e}")
...
def fetch_today_invoices(subs):
    ...
    if now.hour < 12:
        yesterday = yesterday - _td(days=1)
    ...
    try:
        ...
        while True:
            page = _stripe_call(stripe.Event.list, **params)
...
def _fetch_month_collected(mi):
    ...
    try:
        ...
        while True:
            page = _stripe_call(stripe.Invoice.list, **params)
...
def fetch_monthly_collected():
    ...
    if cache_updated:
        try:
            cache["__version__"] = CACHE_VER  # was hardcoded "v2" — caused cache to never be reused
